...
void loop(){
  ...
  cmE = 0.01723 * distaE(triggerPinE, echoPinE);
    ...
    mediaSensor= (cmF+cmE+cmD)/3.0;   //IREI TENTAR TRANSFORMAR OS TRES SENSORERS EM APENAS UM,PARA ISSO IREI FAZER A MEDIA DAS DISTANCIAS DOS TRES.
  ...
  if (mediaSensor >= 3 && mediaSensor<= 100 && eixoX>=320 && eixoX<=340 )
  {
  ...
  else if (mediaSensor >= 3 && mediaSensor<= 100 && eixoX>=320 && eixoX<=340)
  {
     xE = map(cmE,3,100,255,120);
